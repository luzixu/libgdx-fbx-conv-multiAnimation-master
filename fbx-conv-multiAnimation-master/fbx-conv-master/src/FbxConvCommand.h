...
struct FbxConvCommand {
	...
	FbxConvCommand(log::Log *log, const int &argc, const char** argv, Settings *settings)
		...
		settings->inType = FILETYPE_AUTO;
             ...
             **/
            settings->deleteNormal=false;
            settings->breakAnimation=false;
            settings->isExtractAnimation=false;
		...
		for (int i = 1; i < argc; i++) {
			...
			if (len > 1 && arg[0] == '-') {
                ...
                else if ((arg[1]==  'd') && (i+1<argc))
                    settings->deleteNormal=true;
                else if((arg[1]=='s') && (i+1<argc)){
                    int num=atoi(argv[++i]);
                    if(i+num*3<argc){
                        parseBreakAnimation(argv, settings,i,num);
                        i+=num*3;
                    }
                }else if(arg[1]=='e'){
                    settings->isExtractAnimation=true;
                }
	...
	void printHelp() const {
        ...
        printf("-d       : Delete the normal\n");
        printf("-s       : Break the animation, the style is \"[animationIds]...[spanLoc]\"\n");
		printf("\n");
...
private:
    ...
    void parseBreakAnimation(const char ** argv, Settings * settings,int start,int num){
        start++;
        for(int i=start;i<start+3*num;i++){
            const char * vaule=argv[i];
            if(i<start+num) settings->breakAnimationIds.push_back(std::string(vaule));
            else  settings->animation_span_loc.push_back(atoi(vaule));
        }
        settings->breakAnimation=true;
    }
