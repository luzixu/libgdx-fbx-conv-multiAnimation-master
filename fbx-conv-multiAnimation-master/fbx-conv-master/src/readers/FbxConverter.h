...
namespace readers {
	...
	public:
		...
		virtual bool convert(Model * const &model) {
             ...
             */
            if(settings->deleteNormal) model->deleteNormal(settings);
            if(settings->breakAnimation) model->breakAnimation(settings);
			return true;
		}
...
//				const float last = (*itr).second.stop + stepSize * 0.5f;
                const float stepSize = 41.666667;
                ...
                if(settings->isExtractAnimation)
                    addKeyframes_extractAniamiton(nodeAnim, frames);
                else
                    addKeyframes_new(nodeAnim,frames);
        ...
        void addKeyframes_new(NodeAnimation *const &anim, std::vector<Keyframe *> &keyframes) {
        ...
        void addKeyframes_extractAniamiton(NodeAnimation *const &anim, std::vector<Keyframe *> &keyframes) {
            bool translate = false, rotate = false, scale = false;
            // Check which components are actually changed
            for (std::vector<Keyframe *>::const_iterator itr = keyframes.begin(); itr != keyframes.end(); ++itr) {
                if (!translate && !cmp(anim->node->transform.translation, (*itr)->translation, 3))
            ...
            if (!keyframes.empty()) {
                ...
                for (int i = 1; i < last; i++) {
                    ...
                    //                           // (!isLerp(k1->rotation, k1->time, k2->rotation, k2->time, k3->rotation, k3->time, 4)) || // FIXME use slerp for quaternions
                    //
                    //                            (!cmp(k1->rotation,k2->rotation,4) || !cmp(k2->rotation,k3->rotation,4))||
                    //                            (!isLerp(k1->scale, k1->time, k2->scale, k2->time, k3->scale, k3->time, 3))) {
                    //                            anim->keyframes.push_back(k2);
                    if ((!cmp(k1->translation,k2->translation,3)) ||
                        (!cmp(k1->rotation,k2->rotation,4) || !cmp(k2->rotation,k3->rotation,4))||
                        (!cmp(k1->scale,k2->scale,3) )) {
                        anim->keyframes.push_back(k2);
                        k1 = k2;
                        countDiff++;
                    } else{
            ...
            printf("-----is same---= %d  diff= %d  node=%s\n" ,countSame,countDiff,anim->node->id.c_str());
        }
